...
if (!empty($_SESSION['user_logged_in']) && !empty($_SESSION['user_id'])) {
    ...
    // If client, redirect to client app
...
<head>
    ...
    <title>Provider Login - Scriptly</title>
...
<body class="bg-brand-bg text-brand-dark font-sans antialiased selection:bg-blue-200 selection:text-blue-900" style="background-color: #f8f7f5;">
        ...
        <main class="lg:col-span-7 flex flex-col justify-between p-6 sm:p-10 bg-white overflow-y-auto">
            ...
            <div class="max-w-sm w-full mx-auto my-auto space-y-5">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1 leading-relaxed">Enter your credentials to access your provider workspace.</p>
                </div>
                ...
                <div class="relative flex items-center justify-center my-2">
                    ...
                    <span class="bg-white px-3 text-[10px] font-extrabold text-gray-400 uppercase tracking-widest absolute">Or log in with email</span>
                </div>
                ...
                <form id="login-form" class="space-y-4" novalidate>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <a href="../forgot-password.php" class="text-[11px] text-blue-600 font-bold hover:underline">Forgot password?</a>
                        </div>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-gray-400">
                    ...
                    <button type="submit" id="submit-btn" class="w-full py-3 bg-[#0A2342] text-white rounded-full font-bold text-xs hover:bg-slate-800 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md mt-4">
                        <span id="btn-text">Authenticate & Open Workspace</span>
                ...
                <div class="pt-4 border-t border-gray-100 text-center text-xs text-gray-500 font-medium">
                    Don't have a provider account? <a href="signup.php" class="text-blue-600 font-bold hover:underline ml-0.5">Apply for free →</a>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Handle Alert message from URL query params
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.has('msg') && urlParams.get('msg') === 'logged_out') {
            showAlert('success', 'Logged out successfully from session.');
        } else if (urlParams.has('msg') && urlParams.get('msg') === 'suspended') {
            showAlert('error', 'Your provider account is currently suspended.');
        }
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    showAlert('success', 'Logged in successfully! Loading Pro workspace...');
                    setTimeout(() => {
                        const host = window.location.host;
                        const protocol = window.location.protocol + '//';
                        const isLocal = window.location.pathname.includes('/creda/');
                        let redirect = data.redirect;
                        if (redirect === 'verify-email' || redirect === 'verify-email.php') {
                            redirect = 'provider/verify-email.php';
                        }
                        let targetUrl = '';
                        if (isLocal) {
                    ...
                    btnText.textContent = 'Authenticate & Open Workspace';
                }
            } catch (err) {
