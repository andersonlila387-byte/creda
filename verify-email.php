...
<head>
    ...
    <title>Verify Email - Scriptly Verified Marketplace</title>
    
    <!-- Primary SEO Meta Tags -->
    <meta name="title" content="Verify Email - Scriptly Verified Marketplace">
    <meta name="description" content="Verify your email address with your 6-digit OTP code to activate your Scriptly account.">
...
<body class="bg-brand-bg text-brand-dark font-sans antialiased selection:bg-blue-200 selection:text-blue-900">
    ...
    <div class="min-h-screen grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
        ...
        <aside class="lg:col-span-5 hidden lg:flex flex-col justify-between text-white p-8 xl:p-10 relative overflow-hidden bg-cover bg-center" style="background-image: url('https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&q=80&w=1200');">
            ...
            <div class="relative z-10 max-w-sm space-y-5">
                ...
                <blockquote class="bg-white/10 backdrop-blur-md rounded-[3px] p-4 border border-white/15 text-xs leading-relaxed text-gray-200">
                    "Email verification ensures every buyer and service provider on Scriptly is authentic, preventing impersonation and fraud."
        ...
        <main class="lg:col-span-7 flex flex-col justify-between p-6 sm:p-10 bg-white overflow-y-auto">
            ...
            <div class="flex items-center justify-between mb-6 text-xs">
                <a href="signup" class="inline-flex items-center text-gray-500 hover:text-brand-dark font-semibold transition-colors">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const fillDebugBtn = document.getElementById('fill-debug-btn');
            ...
            // OTP Input auto-advance & keyboard navigation logic
            ...
            // Local Dev Auto-Fill Helper
            if (fillDebugBtn) {
                fillDebugBtn.addEventListener('click', () => {
                    const codeDisplay = document.getElementById('debug-otp-display').textContent.trim();
                    if (codeDisplay.length === 6) {
                        codeDisplay.split('').forEach((char, i) => {
                            if (otpInputs[i]) otpInputs[i].value = char;
                        });
                        showAlert('info', 'Code auto-filled. Click "Verify Email" to proceed.');
                    }
                });
            }
            ...
            // Resend Code Handler with Loading Spinner Animation & 60s Countdown
            let countdown = 0;
