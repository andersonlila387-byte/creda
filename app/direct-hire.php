...
if (!\ || (\['primary_role'] ?? 'client') !== 'client') {
    header("Location: ../login.php?redirect=direct-hire.php?" . http_build_query(\));
    exit;
}
...
// Fetch Provider Info
\ = "SELECT id, full_name, title FROM users WHERE id = ? LIMIT 1";
\ = \->prepare(\);
\->execute([\]);
...
if (\['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty(\) || \ <= 0) {
        \ = "Please provide a valid project title and budget.";
    } else {
        \ = \ * 0.05; // 5% escrow fee
        \ = \ + \;
        ...
        try {
            ...
            if(!empty(\)) {
                \ = \->prepare("INSERT INTO messages (contract_id, sender_id, receiver_id, content) VALUES (?, ?, ?, ?)");
                \->execute([\, \, \, "Project Description/Requirements: " . \]);
            }
            
            \->commit();
            
            // Redirect to Contract Details
            header("Location: contract-details.php?id=" . \ . "&success=direct_hire");
            exit;
        } catch (Exception \) {
            if (\->inTransaction()) {
                \->rollBack();
            }
            \ = "Booking failed: " . \->getMessage();
        }
    }
}
?>
                ...
                <div class="w-full lg:w-80 shrink-0">
                    <div class="bg-white border border-slate-200 rounded-[3px] shadow-sm overflow-hidden sticky top-6">
                        ...
                        <div class="p-4 text-center">
                            ...
                            <h3 class="text-sm font-bold text-slate-900 leading-snug"><?php echo htmlspecialchars(\['full_name']); ?></h3>
                            <p class="text-[11px] text-slate-500 mt-1 font-semibold"><?php echo htmlspecialchars(\['title']); ?></p>
                        </div>
