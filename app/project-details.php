...
if (!$user_id) {
    header("Location: ../login.php?redirect=project-details.php?slug=" . urlencode($slug));
    exit;
}
...
if ($user_role === 'provider') {
    $chk_stmt = $db->prepare("SELECT id, bid_amount, status FROM proposals WHERE project_id = ? AND provider_id = ? LIMIT 1");
    $chk_stmt->execute([$project['id'], $user_id]);
    $existing_proposal = $chk_stmt->fetch(PDO::FETCH_ASSOC);
}
...
<head>
    ...
    <title><?= htmlspecialchars($project['title']) ?> - Scriptly</title>
</head>
                        ...
                        <div class="bg-white p-5 sm:p-7 rounded-[3px] border border-slate-200/90 shadow-sm">
                            ...
                            <div class="flex flex-wrap gap-y-3 gap-x-6 text-xs text-slate-600 font-medium pb-5 border-b border-slate-100">
                                <div>
                                    ...
                                    <span class="text-slate-800 font-bold"><?= date('M j, Y', strtotime($project['created_at'])) ?></span>
                                </div>
                                <div>
                                    <span class="text-slate-400 block mb-0.5">Proposals</span>
                                    <span class="text-slate-800 font-bold"><?= $project['proposal_count'] ?> received</span>
                                </div>
                                <?php if ($project['deadline_date']): ?>
                                <div>
                                    <span class="text-slate-400 block mb-0.5">Deadline</span>
                                    <span class="text-slate-800 font-bold"><?= date('M j, Y', strtotime($project['deadline_date'])) ?></span>
                                </div>
                                <?php endif; ?>
                            </div>
                    ...
                    <div class="lg:col-span-4 space-y-5 lg:sticky lg:top-4">
                        ...
                        <div class="bg-white p-5 rounded-[3px] border border-slate-200/90 shadow-sm text-center">
                            ...
                            <div class="text-2xl font-black text-slate-900 mb-6">
                                <?= $project['budget'] > 0 ? '₦' . number_format($project['budget']) : 'Open to Bids' ?>
                            </div>
                            ...
                            <?php elseif ($user_role === 'provider'): ?>
                                <?php if ($existing_proposal): ?>
                                    <div class="p-3 bg-blue-50 border border-blue-200 rounded-[3px] mb-3 text-left">
                                        ...
                                        <div class="text-[11px] text-slate-600">You placed a bid of ₦<?= number_format($existing_proposal['bid_amount']) ?>.</div>
                                    </div>
                                ...
                                <?php else: ?>
                                    <a href="submit-proposal.php?project_id=<?= $project['id'] ?>" class="block w-full py-3 bg-[#1952E1] hover:bg-blue-700 text-white text-sm font-bold rounded-[3px] shadow-sm transition-colors">
                                        Submit a Proposal
                                    </a>
                                <?php endif; ?>
                            <?php else: ?>
                                <?php if ($project['client_id'] == $user_id): ?>
                                    <a href="review-proposals.php?id=<?= $project['id'] ?>" class="block w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-[3px] shadow-sm transition-colors">
                                        Review Proposals (<?= $project['proposal_count'] ?>)
                                    </a>
                                <?php else: ?>
                        ...
                        <div class="bg-white p-5 rounded-[3px] border border-slate-200/90 shadow-sm">
                            ...
                            <div class="space-y-3">
                                <div>
                                    <div class="font-bold text-sm text-slate-900"><?= htmlspecialchars($project['client_name']) ?></div>
                                    <div class="text-xs text-slate-500">Member since <?= date('M Y', strtotime($project['client_joined'])) ?></div>
                                </div>
                                <div class="flex items-center gap-2 text-xs font-medium text-slate-700 pt-2 border-t border-slate-50">
                                    ...
                                    <?= $project['client_total_projects'] ?> total projects posted
                                </div>
